...
class AudioSessionManager(private val context: Context) {
            ...
            AudioManager.OnAudioFocusChangeListener { focusChange ->
                when (focusChange) {
                    AudioManager.AUDIOFOCUS_GAIN -> {
                        ...
                        FlutterAudioRecorderPlayerPlugin.instance?.onAudioFocusGained()
                    }
                    AudioManager.AUDIOFOCUS_LOSS -> {
                        Log.d(TAG, "Audio focus lost")
                        // Notify the plugin that audio focus was lost
                        FlutterAudioRecorderPlayerPlugin.instance?.onAudioFocusLost()
                    }
                    AudioManager.AUDIOFOCUS_LOSS_TRANSIENT -> {
                        Log.d(TAG, "Audio focus lost temporarily")
                        // Notify the plugin that audio focus was lost temporarily
                        FlutterAudioRecorderPlayerPlugin.instance?.onAudioFocusLostTransient()
                    }
                    AudioManager.AUDIOFOCUS_LOSS_TRANSIENT_CAN_DUCK -> {
                        ...
                        FlutterAudioRecorderPlayerPlugin.instance
    ...
    fun configure(contentType: Int, focusGainType: Int, pauseWhenDucked: Boolean) {
        Log.d(
                TAG,
                "Configuring audio session: contentType=$contentType, focusGainType=$focusGainType, pauseWhenDucked=$pauseWhenDucked"
        )
    ...
    private fun createAudioAttributes(contentType: Int): AudioAttributes {
        ...
        return AudioAttributes.Builder().setUsage(usage).setContentType(contentTypeAttr).build()
    }
